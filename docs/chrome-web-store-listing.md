...
| `storage` | Stores the first-use disclosure choice locally. Memory-backed session storage, restricted to trusted extension contexts, holds the On Paper pairing credential and a pending application request identifier, opaque profile identifier, and one-way fingerprint used to prevent duplicate saves after an interruption. This retry metadata contains no job details or resume content, is kept separately for each Chrome window, and survives panel close/reopen, disconnect, and re-pairing in that window; a confirmed save, Start over, a verified profile change, or Chrome clearing session storage removes it. The credential is cleared on extension reload, update, disable, or browser restart and is never persisted to Chrome’s local or sync storage. |
...
September 25 update: the owner reports generating a dedicated OpenRouter key with a **$5 budget** and **30-day expiry** and placing it privately in the Dashboard **Password** field, with **Username** blank. Its remaining balance and continued availability have not been independently inspected. The reviewer enters it in the native welcome wizard if needed on first launch, or **On Paper Settings → AI** afterward, and selects the tested **Claude Sonnet 4.6** model. No On Paper login or reviewer-funded OpenRouter account is required for this arrangement.
