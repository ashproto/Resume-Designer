...
- [ ] **Step 4.0 (reuse):** Settings (Task 1) defined in-file `GroupTitle` + `SettingRow` helpers for the rail-dialog pattern. Extract those two into a shared module (e.g. `src/components/ui/setting-row.jsx` or `src/components/settings-primitives.jsx`) and have BOTH SettingsDialog and ProfileDialog import them — the second consumer is the right moment to hoist (per Task 1 code review). Keep `StatCard`/`UsageTable` local to Settings. **Also extract the shared dialog shell**: Settings + History now duplicate the `DialogContent glass-card` scaffold + the ghost-X close button cluster (`-mr-1 -mt-0.5 flex h-7 w-7 … hover:bg-accent`) + the header title/description block, and they express the header hairline on different elements (History `border-b` on header vs Settings `border-t` on body). Extract a `<DialogShell>`/`<DialogCloseX>` (or at least a shared header) used by Settings, History, Profile, Jobs, and standardize which element owns the divider (per Task 2 code review).
