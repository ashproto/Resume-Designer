...
// Change only the display name and scheme references after each generation.
export function brandIosTarget(project, scheme) {
  ...
  if (!configId || !new RegExp(`^\\t\\t${configId} /\\* [^\\n]*_iOS[^\\n]* \\*/ = \\{`, 'm').test(configSection)) {
  ...
  return { project: project.replace(target, brandedTarget), scheme: brandedScheme };
}
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try { main(resolve(process.argv[2] || 'resume-designer.xcodeproj')); }
